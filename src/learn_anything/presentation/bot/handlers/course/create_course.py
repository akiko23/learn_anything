...
from learn_anything.presentation.bot.keyboards.create_course import CANCEL_COURSE_CREATION_KB, GET_COURSE_PHOTO_KB, \
...
async def start_course_creation(
    ...
    await state.update_data(
        msg_on_delete=callback_query.message.message_id + 1
    )

    await bot.delete_message(chat_id=user_id, message_id=callback_query.message.message_id)

    await bot.send_message(chat_id=user_id, text='Введите название курса', reply_markup=CANCEL_COURSE_CREATION_KB)
...
async def get_course_photo(
    ...
    user_id: int = msg.from_user.id
    photo_id: str = msg.photo[0].file_id
    data: dict[str, Any] = await state.get_data()
...
async def get_course_registrations_limit(
    ...
    await bot.send_message(
        chat_id=user_id,
        text='Создание курса завершено',
        reply_markup=after_course_creation_menu(new_course_id=output_data.id),
    )
